...
COIN_WHITELIST = {"XBT", "ETH", "SOL", "BTC", "ADA", "XRP", "AVAX", "LINK", "DOT", "MATIC"}  # v29.6: all-market 10-pair expansion
WHITELIST_ONLY = True
...
MIN_MOMENTUM_SCORE = 0.06       # v29.6: lowered from 0.08 — more signals for broader whitelist
...
TRIPLE_MEANREV_Z_THRESHOLD = 1.5   # v29.6: lowered from 2.0 — more MR signals
TRIPLE_MEANREV_HIGH_CONV_Z = 2.5   # v29.6: lowered from 3.0 — high-conviction threshold
...
# ── v29.6: Aggressive Mean Reversion Signal ──
def mean_rev_aggressive_signal(coin, prices):
    if len(prices) < 25:
        return None
    import statistics
    closes = [p for p in prices[-25:] if p > 0]
    if len(closes) < 20:
        return None
    ...
    if current < lower_bb and rsi < 35 and zscore < -1.5:
        score = min(10, int(abs(zscore) * 2 + (35 - rsi) / 5))
        stop_pct = max(0.5, abs(zscore) * 0.3)
        return ("long", score, current, stop_pct, stop_pct * 3.2)
    if current > upper_bb and rsi > 65 and zscore > 1.5:
        score = min(10, int(zscore * 2 + (rsi - 65) / 5))
        stop_pct = max(0.5, zscore * 0.3)
        return ("short", score, current, stop_pct, stop_pct * 3.2)
    return None
...
def main():
    ...
    while running[0]:
        try:
            ...
            _regime_allows_mr = _current_regime in ("RANGING", "CHOPPY", "QUIET_RANGE", "VOLATILE_TREND") or _is_ranging
            if _regime_allows_mr:
                for r in last_full_rank[:8]:
                    _mr_coin = pair_names.get(r["pair"], r["pair"])
                    _mr_blocked = _mr_coin in DYNAMIC_BLACKLIST
                    if _mr_blocked:
                        continue
                    ...
                    if len(wallet.longs) + len(wallet.shorts) >= MAX_POSITIONS:
                        break
                    _mr_prices = prices_cache.get(_mr_coin, [])
                    if len(_mr_prices) < 20:
                        continue
                    _mr_result = mean_rev_aggressive_signal(_mr_coin, list(_mr_prices))
                    if _mr_result is None:
                        continue
                    _mr_dir, _mr_score, _mr_entry, _mr_stop, _mr_tp = _mr_result
                    if _mr_score < 5:
                        continue
                    _mr_pv = wallet.portfolio_value()
                    _mr_size = max(MIN_ORDER_USD, _mr_pv * MAX_RISK_PER_TRADE * 0.5)
                    _mr_size = min(_mr_size, _mr_pv * MAX_SINGLE_COIN_EXPOSURE_PCT / 100)
                    if _mr_size < MIN_ORDER_USD:
                        continue
                    if _mr_dir == "long" and _mr_coin not in wallet.longs:
                        wallet.buy(_mr_coin, _mr_size, _mr_entry, "mean_rev_aggressive",
                                   stop_loss=_mr_entry*(1-_mr_stop/100),
                                   take_profit=_mr_entry*(1+_mr_tp/100))
                        logger.info(f"[MR_ENTRY] {_mr_coin} LONG score={_mr_score} | ${_mr_size:.2f}")
                    elif _mr_dir == "short" and _mr_coin not in wallet.shorts:
                        wallet.short(_mr_coin, _mr_size, _mr_entry, "mean_rev_aggressive",
                                     stop_loss=_mr_entry*(1+_mr_stop/100),
                                     take_profit=_mr_entry*(1-_mr_tp/100))
